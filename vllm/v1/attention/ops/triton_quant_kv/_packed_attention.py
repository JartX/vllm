...
The :func:`_attn_packed` kernel splits the head dimension into two
interleaved INT4 nibble streams (``PACKING_FACTOR=2``), unpacks them from
the packed KV cache, applies the asymmetric zero-point score correction,
and runs the shared online-softmax tile loop / 2D-3D epilogue.
"""
...
def _attn_packed(
    ...
    IS_3D: tl.constexpr,
    # 2 → INT4 nibble pair (asymmetric + zp).
    ...
    # -----------------------------------------------------------------
    # Split-Q prologue: 2 interleaved INT4 streams of Q (even / odd).
    # The packed KV cache stores one byte per ``packed_offs``, holding
    # PACKING_FACTOR (= 2) values.
    # -----------------------------------------------------------------
    ...
    for j in range(loop_lo, loop_hi):
        ...
        # Fuse v per-(token, head) scale into P, subtracting the
        # v-zero-point contribution from each INT4 stream once.
