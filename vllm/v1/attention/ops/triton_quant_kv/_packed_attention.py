...
"""Attention (read-path) kernel + launcher for the sub-byte packed mode.

:func:`_attn_packed` handles INT4 (``PACKING_FACTOR=2``): the packed KV is
read as two interleaved nibble streams, split-dotted against the matching
Q streams, and the asymmetric zero-point correction is applied on the
score side.  Prologue, masking, online softmax, tile loop and the 2D/3D
epilogue are shared with the flat per-token-head kernels.
"""
...
def _attn_packed(
    ...
    IS_3D: tl.constexpr,
    # 2 → INT4 nibble pair (asymmetric + zp).  The packed loads below
    # gate on
    # this value.
    ...
    # -----------------------------------------------------------------
    # Split-Q prologue: PACKING_FACTOR interleaved streams of Q (even /
    # odd for INT4).  The packed KV cache stores one byte per
    # ``packed_offs``, which holds PACKING_FACTOR values.
    # -----------------------------------------------------------------
    ...
    for j in range(loop_lo, loop_hi):
        ...
        # softmax_scale * per-(token, head) k_scale in one mul, minus the
        # ``zp * sum(Q)`` asymmetric correction term.
        ...
        # Fuse v per-(token, head) scale into P, then subtract the
