...
"""Per-mode KV cache quantization factories.
...
``FP8_PER_TOKEN_HEAD`` directly via constexpr branches.  Factories
registered here own:

  * the **write side** for any mode that needs more than a plain copy
    (per-token-head absmax, asymmetric INT4 with zero-point packing); and
  * the **attention read side** for the sub-byte packed INT4 mode
    whose inner loop is structurally different from the core kernel
    (split-dot, sub-byte unpack, etc.).
...
3. Create a new file under ``quant_kv/`` that defines a subclass of
   :class:`QuantKVFactory` and calls :func:`register` at module level.
...
_REGISTRY: dict[KVQuantMode, QuantKVFactory] = {}
...
def register(factory: QuantKVFactory) -> None:
    if factory.mode in _REGISTRY:
        existing = _REGISTRY[factory.mode].__class__.__name__
        new = factory.__class__.__name__
        if existing != new:
            raise RuntimeError(
                f"Duplicate QuantKVFactory registration for "
                f"{factory.mode.name}: {existing} vs {new}"
            )
        return
    _REGISTRY[factory.mode] = factory


def get_quant_kv_factory(mode: KVQuantMode) -> QuantKVFactory:
    """Lazy-import and return the factory for *mode*.

    Raises ``ValueError`` if no factory module is configured for *mode*.
    """
    if mode == KVQuantMode.NONE:
        raise ValueError("KVQuantMode.NONE is the unquantized path and has no factory")
    if mode not in _REGISTRY:
        module_path = _MODULES.get(mode)
        if module_path is None:
            raise ValueError(
                f"No QuantKVFactory module configured for {mode.name}.  "
        ...
        if mode not in _REGISTRY:
            raise RuntimeError(
                f"Module {module_path} did not register a factory for "
                f"{mode.name}.  Each factory module must call "
                f"`quant_kv.register(MyFactory())` at the bottom."
            )
    return _REGISTRY[mode]


def has_quant_kv_factory(mode: KVQuantMode) -> bool:
    """Return True if *mode* has a factory (loaded or lazily available)."""
