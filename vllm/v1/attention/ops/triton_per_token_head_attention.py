...
def _pth_attn_stage1_packed(
    ...
    INT4 (``PACKING_FACTOR=2``).  Q arrives pre-rotated (RHT for INT4) so
    the dot is computed in the same rotated space the cache was written in.
    ...
    for start_n in range(split_start, split_end, BLOCK_KV):
        ...
        # Per-stream PV accumulation, subtracting the v-zp term once.
        pv = p * v_scales
...
def triton_per_token_head_attention(
    ...
    * INT4 per-token-head — sub-byte packed cache.  Q is rotated
      (RHT for INT4) before the kernel; the
    ...
    if use_grouped_wmma:
        ...
        # tightest head_size + packing combos (e.g. head_size=64 + INT4).
...
def _maybe_rotate_q(q: torch.Tensor, kv_quant_mode: KVQuantMode) -> torch.Tensor:
    """RHT for INT4 via HIP butterfly inplace, Triton single-RHT fallback."""
    if kv_quant_mode == KVQuantMode.INT4_PER_TOKEN_HEAD:
        if (
...
def triton_per_token_head_prefill(
    ...
    * INT4 per-token-head — sub-byte packed cache.  Q is rotated
    ...
    if packing_factor > 1:
        ...
        # which for INT4 + D=256 grows large and can spill to scratch on
