...
# Per-expert planes fed to the GEMM (all indexed by expert_id on dim 0).
_EXPERT_PLANES = (
    "w13_weight_packed",
    "w2_weight_packed",
    "w13_weight_scale",
...
class CompressedTensorsWNA16RDNA3MoEMethod(CompressedTensorsWNA16MoEMethod):
    ...
    def _process_weights_offload(self, layer: torch.nn.Module) -> None:
        ...
        layer.rdna3_act_buf = torch.empty(buf, intermediate, dtype=act_dtype,
                                          device=device)
        ...
        ) + layer.w13_qzeros.numel() * 4 + layer.w2_qzeros.numel() * 4
        logger.info_once(
            "[expert-offload] cache C=%d / E=%d experts, "
            "%.0f MB/layer (%.1f GB/rank over %d layers est.)",
            C, E, per_layer / 1e6, per_layer * 48 / 1e9, 48,
        )
