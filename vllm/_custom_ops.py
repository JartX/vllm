...
# Diagnostic-only WMMA fragment-layout probe (RDNA3). Returns fp32 [32, 8].
def gptq_gemm_rdna3_wmma_probe(
    a: torch.Tensor, b: torch.Tensor, mode: int
) -> torch.Tensor:
    return torch.ops._C.gptq_gemm_rdna3_wmma_probe(a, b, mode)


if hasattr(torch.ops._C, "gptq_gemm_rdna3_wmma_probe"):

    @register_fake("_C::gptq_gemm_rdna3_wmma_probe")
    def _gptq_gemm_rdna3_wmma_probe_fake(
        a: torch.Tensor, b: torch.Tensor, mode: int
    ) -> torch.Tensor:
        return torch.empty((32, 8), dtype=torch.float32, device=a.device)


# Diagnostic-only c_acc dump from full gemm_q4_wmma path. Returns fp32 [32, 8].
def gptq_gemm_rdna3_wmma_dump(
    a: torch.Tensor,
    b_q_weight: torch.Tensor,
    b_qzeros: torch.Tensor,
    b_scales: torch.Tensor,
    ...
    return torch.ops._C.gptq_gemm_rdna3_wmma_dump(
        a, b_q_weight, b_qzeros, b_scales, use_v2_format
    )


if hasattr(torch.ops._C, "gptq_gemm_rdna3_wmma_dump"):

    @register_fake("_C::gptq_gemm_rdna3_wmma_dump")
    def _gptq_gemm_rdna3_wmma_dump_fake(
        a: torch.Tensor,
        b_q_weight: torch.Tensor,
        b_qzeros: torch.Tensor,
        b_scales: torch.Tensor,
...
# Diagnostic-only LDS dump (dequant + LDS-write only). Returns fp16 [16, 16].
def gptq_gemm_rdna3_wmma_lds_check(
    b_q_weight: torch.Tensor,
    b_qzeros: torch.Tensor,
    b_scales: torch.Tensor,
    use_v2_format: bool,
) -> torch.Tensor:
    return torch.ops._C.gptq_gemm_rdna3_wmma_lds_check(
        b_q_weight, b_qzeros, b_scales, use_v2_format
    )


if hasattr(torch.ops._C, "gptq_gemm_rdna3_wmma_lds_check"):

    @register_fake("_C::gptq_gemm_rdna3_wmma_lds_check")
    def _gptq_gemm_rdna3_wmma_lds_check_fake(
        b_q_weight: torch.Tensor,
        b_qzeros: torch.Tensor,
        b_scales: torch.Tensor,
        use_v2_format: bool,
        ...
        return torch.empty(
            (16, 16), dtype=torch.float16, device=b_q_weight.device
        )
