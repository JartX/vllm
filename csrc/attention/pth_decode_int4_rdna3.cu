...
__global__ void decode_int4_reduce_v2(
  ...
  for (int s = 0; s < num_splits; ++s) {
    ...
    // NOT __expf(delta): __expf computes e^(log2_delta) and shrinks non-max
    // splits by ~e^(-0.307*|delta|), biasing attention toward the max split
    // and inducing repetition loops on long generations.
