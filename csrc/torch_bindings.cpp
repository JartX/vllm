...
TORCH_LIBRARY_EXPAND(TORCH_EXTENSION_NAME, ops) {
  ...
  // W4A16 GPTQ GEMM tuned for AMD RDNA3 (gfx1100). Accepts both fp16 and
  // bf16 activations; weights must be pre-shuffled via gptq_shuffle.
  ops.def(
  ...
  // RDNA3 WMMA prefill kernel + diagnostics. Registered but NOT called from
  // Python in the current bisect step (Python dispatch unchanged). Tests
  // whether the TU compile alone causes the decode regression.
  ops.def(
